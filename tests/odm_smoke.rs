...
use onvif_server::{
    DeviceService, DeviceServiceHandler, EventService, EventServiceHandler, ImagingService,
    ImagingServiceHandler, ImagingSettings, OnvifError,
};
...
async fn odm_smoke_event_lifecycle() {
    ...
    assert!(
        unsub_result.is_ok(),
        "Unsubscribe must return Ok, got: {:?}",
        unsub_result.err()
    );
}
...
async fn odm_smoke_full_sequence() {
    ...
    // 1. GetCapabilities
    let caps = device_handler
        .handle(Bytes::from_static(
            b"<tds:GetCapabilities xmlns:tds=\"http://www.onvif.org/ver10/device/wsdl\"/>",
        ))
        .await
        .expect("GetCapabilities must not fault");
    let caps_xml = String::from_utf8(caps.to_vec()).unwrap();
    assert!(
        caps_xml.contains("imaging_service"),
        "full_sequence: GetCapabilities missing imaging_service"
    );
    assert!(
        caps_xml.contains("events_service"),
        "full_sequence: GetCapabilities missing events_service"
    );

    // 2. GetDeviceInformation
    let devinfo = device_handler
        .handle(Bytes::from_static(
            b"<tds:GetDeviceInformation xmlns:tds=\"http://www.onvif.org/ver10/device/wsdl\"/>",
        ))
        .await
        .expect("GetDeviceInformation must not fault");
    let devinfo_xml = String::from_utf8(devinfo.to_vec()).unwrap();
    assert!(
        devinfo_xml.contains("Manufacturer"),
        "full_sequence: GetDeviceInformation missing Manufacturer"
    );

    // 3. GetServices
    let svcs = device_handler
        .handle(Bytes::from_static(
            b"<tds:GetServices xmlns:tds=\"http://www.onvif.org/ver10/device/wsdl\">\
          <tds:IncludeCapability>false</tds:IncludeCapability>\
          </tds:GetServices>",
        ...
        .await
        .expect("GetServices must not fault");
    let svcs_xml = String::from_utf8(svcs.to_vec()).unwrap();
    assert!(
        svcs_xml.contains("ver20/imaging"),
        "full_sequence: GetServices missing imaging namespace"
    );
    assert!(
        svcs_xml.contains("ver10/events"),
        "full_sequence: GetServices missing events namespace"
    );

    // 4. GetImagingSettings
    let img = imaging_handler
        .handle(Bytes::from_static(
            b"<timg:GetImagingSettings xmlns:timg=\"http://www.onvif.org/ver20/imaging/wsdl\">\
          <timg:VideoSourceToken>VideoSourceToken</timg:VideoSourceToken>\
          </timg:GetImagingSettings>",
        ...
        .await
        .expect("GetImagingSettings must not fault");
    let img_xml = String::from_utf8(img.to_vec()).unwrap();
    assert!(
        img_xml.contains("timg:ImagingSettings"),
        "full_sequence: GetImagingSettings missing timg:ImagingSettings"
    );
    ...
    let sub_id = {
        ...
        let end = create_xml[start..]
            .find(end_tag)
            .expect("no SubscriptionId close")
            + start;
        create_xml[start..end].to_string()
    };
    ...
    event_handler
        ...
        .await
        .expect("PullMessages must not fault");
    ...
    event_handler
        ...
        .await
        .expect("Unsubscribe must not fault");
}
